...
public class EnhanceScrollViewDragController : MonoBehaviour
{
    ...
    /// Process Mouse Input
    /// </summary>
    private void ProcessMouseInput()                           // MouseInput 的方法 
    {
        if (Input.GetMouseButtonDown(0))                            // 在点击鼠标左键的情况下 ， 执行位置的变化 
        {
            if (targetCamera == null)
                return;
            dragTarget = RayCast(this.targetCamera, Input.mousePosition);  // 方法是RayCast  
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            Vector2 delta = cachedPosition - lastPosition;                     //  计算2D的位置 变化  
        ...
        if (Input.GetMouseButtonUp(0))                      // 抬起来的时候  左键 
        {
    ...
    /// Process Touch input
    /// </summary>
    private void ProcessTouchInput()            // 应该在这里调Touch的操作   是触摸屏的操作 
    {
